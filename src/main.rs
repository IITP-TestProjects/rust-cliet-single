...
fn main() {
    ...
    let seeds = vec!["seed-node1", "seed-node2", "seed-node3",
                     "seed-node4", "seed-node5", "seed-node6",
                     "seed-node7", "seed-node8", "seed-node9", "seed-node10"];
    ...
    let pub_keys = vec![pub_key1, pub_key2, pub_key3, pub_key4, 
        pub_key5, pub_key6, pub_key7, pub_key8, pub_key9, pub_key10];
    ...
    let sig1 = sign::cosign(&priv_key1, &secretR1.r, message.as_bytes(), &agg_pk, &agg_commit);
    let sig2 = sign::cosign(&priv_key2, &secretR2.r, message.as_bytes(), &agg_pk, &agg_commit);
    let sig3 = sign::cosign(&priv_key3, &secretR3.r, message.as_bytes(), &agg_pk, &agg_commit);
    let sig4 = sign::cosign(&priv_key4, &secretR4.r, message.as_bytes(), &agg_pk, &agg_commit);
    let sig5 = sign::cosign(&priv_key5, &secretR5.r, message.as_bytes(), &agg_pk, &agg_commit);
    let sig6 = sign::cosign(&priv_key6, &secretR6.r, message.as_bytes(), &agg_pk, &agg_commit);
    let sig7 = sign::cosign(&priv_key7, &secretR7.r, message.as_bytes(), &agg_pk, &agg_commit);
    let sig8 = sign::cosign(&priv_key8, &secretR8.r, message.as_bytes(), &agg_pk, &agg_commit);
    let sig9 = sign::cosign(&priv_key9, &secretR9.r, message.as_bytes(), &agg_pk, &agg_commit);
    let sig10 = sign::cosign(&priv_key10, &secretR10.r, message.as_bytes(), &agg_pk, &agg_commit);
